...
## Doc Guardian (Mandatory)

After completing any phase plan that modifies files in `packages/core/src/`, `packages/typeorm/src/`, or `apps/test-app/src/`, you MUST run the doc-guardian skill before marking the plan complete:

1. Read `.claude/skills/doc-guardian/SKILL.md` for the full workflow
2. Read `.claude/skills/doc-guardian/rules/doc-coverage.md` for the source-to-doc mapping
3. Check which docs are affected by the changed source files
4. Update affected docs to reflect the new API surface
5. Run `cd docs && pnpm build` to verify the docs build

This is not optional. Stale docs are a release blocker.
